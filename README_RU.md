...
| `--search-jobs 8` | одновременных запросов метаданных при поиске |
...
поэтому `--rps` по умолчанию выключен: гейт разносит *старты* запросов и тем
самым сериализует ровно то, что `--search-jobs` пытается совместить:

| `--rps` | `--search-jobs` | один запрос |
|---|---|---|
| 12 | 1 | 5.45 с |
| 12 | 8 | 2.22 с |
| 0 | 8 | **0.75 с** |
| 0 | 16 | 1.88 с — лучше не становится |
