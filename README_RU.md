...
| `--search-jobs 6` | одновременных запросов; больше — хуже качество |
...
| `--search-jobs` | на трек | потеряно метаданных |
|---|---:|---:|
| 1 (по одному) | 9.0 с | 0 |
| 4 | 1.68 с | 0 |
| **6** (по умолчанию) | **1.31 с** | **0** |
| 8 | 0.82 с | 59 |
| 16 | 0.82 с | 308 — треть библиотеки ложно «не найдена» |

шесть — это точка перегиба. канал метаданных лимитирован куда жёстче http-эндпоинтов,
и дальше он отказывает быстрее, чем повторы успевают это закрыть. отказы не видны
как ошибки: попадание, метаданные которого не пришли, — это просто кандидат, до
которого не дошёл скоринг, и трек возвращается ненайденным. скорость выше шести
покупается качеством сопоставления, поэтому по умолчанию не больше.
